...
const ACCENTS = {
  blue:     { c: "oklch(0.55 0.17 250)", soft: "oklch(0.93 0.04 250)" },
...
const BGS = {
  cloud: { bg: "#fbfbfd", soft: "#f5f5f7", rule: "#e3e3e8", ruleSoft: "#ededf1" },
  white: { bg: "#ffffff", soft: "#f5f5f7", rule: "#e5e5ea", ruleSoft: "#efeff3" },
  mist:  { bg: "#f5f5f7", soft: "#ececf0", rule: "#dedee4", ruleSoft: "#e7e7ed" },
};
...
const ACCENT_KEYS = ["blue", "slate", "forest", "charcoal"];
...
function TopNav({ lang, setLang }) {
  ...
  const nav = COPY.nav;
  const items = ["about", "research", "problems", "publications", "talks", "cv", "contact"];
  return (
  ...
  return (
    <section id="about" className="shell">
      <div className="content">
        <div className="section-label">{t(a.label, lang)}</div>
        <h2 className="statement-head">{t(a.head, lang)}</h2>
        <p className="statement-body">{t(a.body, lang)}</p>
      </div>
    </section>
  );
}
...
function Problems({ lang }) {
  ...
  return (
    <section id="problems" className="shell">
      <div className="content">
        <div className="section-label">{t(pr.label, lang)}</div>
        <h2>{t(pr.head, lang)}</h2>
        <p className="talks-intro">{t(pr.intro, lang)}</p>
        <ol className="problems-list">
          {pr.items.map((it, i) => (
            <li key={i} className="problem">
              <span className="problem-n">{String(i + 1).padStart(2, "0")}</span>
              <span className="problem-q">{t(it, lang)}</span>
            </li>
          ))}
        </ol>
      </div>
  ...
  return (
    <li className="paper" data-group={paper.group}
      ...
      <div className="paper-main">
        <h3 className="paper-title">{t(paper.title, "en")}</h3>
    ...
    <>
      ...
      <aside className="drawer" data-open={open ? "1" : "0"} aria-hidden={!open}>
        ...
        <div className="drawer-body">
          <h2 className="drawer-title">{t(cur.title, "en")}</h2>
      ...
      <div>
        ...
        <div className="talk-venue">
          ...
          {talk.link && (
            <>
              {" "}
              <a className="talk-link" href={talk.link} target="_blank" rel="noreferrer">
                {lang === "en" ? "Event page" : "Página do evento"} ↗
              </a>
            </>
          )}
        </div>
...
function App() {
  ...
  useEffect(() => {
    ...
    const b = BGS[tweaks.bg] || BGS.cloud;
